...
-   auto-covariance functional $$F^{x\phi}=\int_0^Tdt \int_t^Tds \frac{\mathbb{E[dx_td\phi_t(s)]}}{dt}$$
...
$$
 v_u = \mathbb{E}^\mathbb{P}[v_u | \mathcal{F}_t] 
\mathcal{E}(\delta Z_t^\mathbb{P}(u)) 
\\

= \mathbb{E}^\mathbb{P}[v_u | \mathcal{F}_t] 
\exp \{ \delta \sqrt{2H} \times \int_t^u \frac{1}{(u-t)^\kappa}dW_s^\mathbb{P} - \frac{\delta^2}{2} (u-t)^{2H}\} 
\\

= \mathbb{E}^\mathbb{P}[v_u | \mathcal{F}_t] \mathcal{E}(\delta Z_t^\mathbb{Q}(u))
\times \exp{\delta \sqrt{2H} \int _t ^ u \frac{\lambda_s}{(u-s)^\kappa}ds}
$$
...
$$v_u = \mathbb{E}^\mathbb{P}[v_u | \mathcal{F}_t] \mathcal{E}(\delta Z_t ^ \mathbb{Q}(u)) \times \exp \{\delta \sqrt{2H} \int_t^u \frac{\lambda(s)}{(u-s)^\kappa} ds \} \\ =\phi_t(u) \mathcal{E}(\delta Z_t^\mathbb{Q}(u))$$ (I assumed they have a typo in the thesis)
